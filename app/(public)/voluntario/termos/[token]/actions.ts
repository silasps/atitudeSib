...
export async function requestOtpVoluntarioAction(
  token: string,
  email: string,
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log(`[Voluntário OTP] código=${otp} para ${email} (Resend não configurado)`)
    return {}
  }
...
<tr><td style="padding:36px 40px;">
  <p style="margin:0 0 20px;font-size:15px;color:#333;">Seu código de confirmação para assinar o termo de voluntariado é:</p>
  <div style="background:#f8fafc;border:2px dashed #e2e8f0;border-radius:12px;padding:24px;text-align:center;margin-bottom:24px;">
    <p style="margin:0;font-size:40px;font-weight:800;color:#15803d;letter-spacing:8px;">${otp}</p>
    <p style="margin:8px 0 0;font-size:12px;color:#94a3b8;">Válido por 15 minutos</p>
  </div>
  <p style="margin:0;font-size:13px;color:#94a3b8;">Se você não solicitou este código, ignore este e-mail.</p>
</td></tr>
</table></td></tr></table>
</body></html>`
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, to: email, subject: `${otp} — Código para assinar o termo de voluntariado`, html }),
    })
  } catch (err) {
    console.error('[Voluntário OTP] Erro ao enviar e-mail:', err)
    return { error: 'Erro ao enviar o código. Tente novamente.' }
  }
  return {}
}

export async function signTermoVoluntarioAction(
  token: string,
  otp: string,
  name: string,
  email: string,
  ...
  const evidence = {
    ...
    termos_assinados: TERMOS_VOLUNTARIO_CONTENT.termos.map(t => t.id),
    autoriza_imagem: autorizaImagem,
    otp_confirmed: true,
    lei_referencia: 'Lei 14.063/2020 + Lei 9.608/1998',
  }
...
<tr><td style="background:#15803d;padding:28px 40px;text-align:center;">
  <p style="margin:0;font-size:20px;font-weight:700;color:#fff;">Projeto Social</p>
  <p style="margin:6px 0 0;font-size:12px;color:rgba(255,255,255,0.6);">Termo de voluntariado assinado</p>
</td></tr>
<tr><td style="padding:36px 40px;">
  <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:12px;padding:20px;text-align:center;margin-bottom:24px;">
    ...
    <p style="margin:8px 0 0;font-size:15px;font-weight:600;color:#166534;">Termo de adesão ao voluntariado assinado</p>
  </div>
  <p style="margin:0 0 24px;font-size:14px;color:#555;line-height:1.7;">
    Olá, <strong>${para}</strong>! O termo de adesão ao voluntariado foi assinado eletronicamente.
  </p>
  <table width="100%" style="background:#f8fafc;border-radius:8px;padding:16px 20px;">
    <tr><td style="font-size:12px;color:#64748b;padding:4px 0;">Signatário</td><td style="font-size:13px;font-weight:600;text-align:right;padding:4px 0;">${signerName}</td></tr>
    <tr><td style="font-size:12px;color:#64748b;padding:4px 0;">Data e hora</td><td style="font-size:13px;text-align:right;padding:4px 0;">${dateStr}</td></tr>
    <tr><td style="font-size:12px;color:#64748b;padding:4px 0;">Registro</td><td style="font-size:11px;color:#64748b;text-align:right;padding:4px 0;">${token.slice(0, 16)}…</td></tr>
  </table>
  <p style="margin:16px 0 0;font-size:12px;color:#94a3b8;line-height:1.6;">
    Validade jurídica: Lei 14.063/2020 + Lei 9.608/1998 (Lei do Voluntariado).
  </p>
</td></tr>
</table></td></tr></table>
</body></html>`
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, to: signerEmail, subject: 'Termo de voluntariado assinado — bem-vindo(a)!', html: html(signerName.split(' ')[0]) }),
    })
    if (adminEmail) {
      await fetch('https://api.resend.com/emails', {
        method: 'POST',
        headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ from, to: adminEmail, subject: `[Voluntário] Termo assinado por ${signerName}`, html: html('Administrador') }),
      })
    }
  } catch (err) {
    console.error('[Voluntário] Erro ao enviar confirmação:', err)
  }
}
